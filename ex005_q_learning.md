...
Q(s,a)を更新できるという制約があった．
本実験のQ学習は，エピソードの終了を待たず，1ステップの遷移が得られるたびに，
「まだ学習途中の $`Q`$ 自身の推定値」を使って即座に $`Q(s,a)`$ を更新する．
このように，最終的な結果を待たずに自分自身の途中の推定値を使って更新することを
**ブートストラップ(bootstrap)**と呼ぶ．
教師あり学習では，教師信号(正解ラベル)は学習の途中で変わらない固定値であるのに対し，
TD学習の「教師信号」に相当する部分(後述のTD目標)は，学習中の $`Q`$ 自身から計算される
点が大きく異なる．
ex004との主な差分は，この「1ステップごとの更新」の1点のみであり，
...
Q学習は，1回の遷移 $`(s_t, a_t, r_{t+1}, s_{t+1})`$ (時刻 $`t`$ で状態 $`s_t`$ において行動
$`a_t`$ を取り，報酬 $`r_{t+1}`$ を得て状態 $`s_{t+1}`$ に移った，という1手分の記録)が
得られるたびに，以下のTD誤差(Temporal Difference誤差)を計算する．

$$
\delta_t = r_{t+1} + \gamma \max_{a'} Q(s_{t+1}, a') - Q(s_t, a_t)
$$

右辺の $`r_{t+1} + \gamma \max_{a'} Q(s_{t+1}, a')`$ を**TD目標**と呼ぶ．
これは「今得た報酬 $`r_{t+1}`$ に，次状態 $`s_{t+1}`$ で最善の行動を取った場合の
(割引された)価値を足したもの」であり，$`Q(s_t,a_t)`$ が本来こうあってほしい値の見積もりに
相当する．TD誤差 $`\delta_t`$ は，この見積もり(TD目標)と現在の推定値 $`Q(s_t,a_t)`$ の差であり，
教師あり学習における「予測値と教師信号の差」に近い役割を果たす．
このTD誤差を使って，学習率 $`\alpha \in (0,1]`$ で

$$
Q(s_t, a_t) \leftarrow Q(s_t, a_t) + \alpha \, \delta_t
$$

と，$`Q(s_t,a_t)`$ をTD誤差の方向に少しだけ近づける．
エピソードが $`s_t`$ で終了する場合は，その先の状態が存在しないため $`\max_{a'} Q(s_{t+1}, a')`$
の項を0として扱う．

TD目標の $`\max_{a'} Q(s_{t+1}, a')`$ は，実際に次状態で選択した行動 $`a_{t+1}`$ に依存せず，
「次状態で(現時点で)最善と思われる行動を取った場合」の価値を用いる．
これは，行動選択に使う方策($`\epsilon`$-greedy，探索を含む)と，価値を見積もるために
仮定する方策(常に貪欲，$`\max`$)が異なることを意味し，このような手法を**off-policy(方策外)**
と呼ぶ．一方，ex004のモンテカルロ制御は，行動選択と価値の見積もりに同じ方策
($`\epsilon`$-greedy)を用いており，これを**on-policy(方策内)**と呼ぶ．
コード上では，`target`が $`r_{t+1} + \gamma \max_{a'} Q(s_{t+1},a')`$ (TD目標)に，
...
10,000エピソード学習後の最終評価(n=1000，3シード平均±標準偏差，学習時間は3シード平均約2.6秒)は
次の通りである．
...
1. 同じ学習エピソード数(10,000)において，Q学習とモンテカルロ制御のどちらが速く高い勝率に到達したか．1ステップごとの更新(TD学習)とエピソード終了後の更新(モンテカルロ法)の違いと結びつけて説明せよ．
...
Q学習はTD誤差を用いてステップごとにQ(s,a)を更新する．
そのため1エピソードあたりの更新回数がモンテカルロ法よりも多い．
収益が確定するまで待つ必要もないため，同じエピソード数でより速く学習が進む傾向がある．
一方で，TD学習は $`\max_{a'}Q(s_{t+1},a')`$ という「まだ学習途上の推定値」を
ブートストラップに用いる．そのため，学習初期には推定誤差が伝播しやすいという特性もある．

動的計画法(ex002・ex003)・モンテカルロ法(ex004)・Q学習(本実験)は，いずれも
同一のベルマン方程式に由来する枠組みに基づく．理論上は同一の最適方策に収束する．
違いは次の2つの軸に整理できる．

- 環境モデルを使うか(動的計画法)，実際のサンプルのみを使うか(モンテカルロ法・Q学習)．
- エピソード全体の収益を使うか(モンテカルロ法)，1ステップごとにブートストラップするか(Q学習)．

この整理が，本教材全体を通じた強化学習の基礎的な分類体系である．
...
**比較する対象**: 各 $`\alpha`$ における学習曲線の形状と，10,000エピソード終了時点の最終評価．
