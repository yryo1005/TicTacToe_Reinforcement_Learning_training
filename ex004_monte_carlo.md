...
本実験では，`TicTacToeEnv.transition_model`(既知の環境モデル)は使用せず，
`TicTacToeEnv.step`によって実際に後手(固定ランダム方策)と対戦しながら生成される
エピソード $ (s_0, a_0, r_1, s_1, a_1, r_2, \ldots, s_{T-1}, a_{T-1}, r_T) $ のみを用いる．

行動選択には $ \epsilon $-greedy方策を用いる．確率 $ \epsilon $ で一様ランダムに行動し，
確率 $ 1-\epsilon $ で現在の $ Q(s,a) $ が最大となる行動を選択する．これは，
既知の最良の行動を選び続ける「活用」と，未知の行動を試す「探索」のバランスを取るためである．

### 2.2 目的関数・更新式

エピソード内の時刻 $ t $ における収益(割引累積報酬)は

$$
G_t = \sum_{k=0}^{T-t-1} \gamma^{k} r_{t+k+1}
$$

と定義される．初回訪問モンテカルロ法(first-visit MC)では，各エピソード内で
状態行動対 $ (s,a) $ が初めて出現した時刻 $ t $ の収益 $ G_t $ のみを用い，
これまでに観測した収益の標本平均によって $ Q(s,a) $ を更新する．

$$
Q(s,a) \leftarrow \frac{1}{N(s,a)} \sum_{i=1}^{N(s,a)} G_t^{(i)}
$$

ここで $ N(s,a) $ は $ (s,a) $ が初回訪問された回数，$ G_t^{(i)} $ はその$ i $回目の収益である．
コード上では，`returns_sum[(s,a)]`が上式の分子($ G_t $の総和)，`returns_cnt[(s,a)]`が
$ N(s,a) $に，`Q[(s,a)]`が更新後の標本平均に対応する．

このように，モンテカルロ法は「実際に得られたエピソード全体の収益」という
バイアスのないサンプルから $ Q(s,a) $ を直接推定する点で，
1step先の期待値を用いる動的計画法(ex002・ex003)と本質的に異なる．
...
| 項目 | 値 | 備考 |
| :--- | :--- | :--- |
| 環境 | 3目並べ(先手Xがエージェント，後手Oは一様ランダム方策) | ex001〜ex003と共通．ただしモデルは使用しない |
| 割引率 $ \gamma $ | 0.95 | ex002・ex003と同一 |
| 探索率 $ \epsilon $ | 0.1 | 固定相手に対する探索と活用のバランスを取るための標準的な値 |
| 学習エピソード数 | 50,000 | 到達可能な状態行動対の訪問回数を十分に確保するため |
| 評価間隔 | 2,000エピソードごと(評価は300エピソード) | 学習曲線を追跡しつつ計算コストを抑えるため |
| 最終評価エピソード数 | 1,000 | ex001〜ex003と同一 |
| シード | 0, 1, 2 | |
...
以下のセルを実行すると，学習エピソード数に対する勝率の学習曲線(3シード平均±標準偏差)と，
学習終了後の最終評価が得られる．
...
モンテカルロ法は環境モデルを必要としない代わりに，方策を評価するために
実際に多数のエピソードをサンプリングする必要があり，動的計画法と比べてはるかに多くの
データ(エピソード数)を要する．これは，モンテカルロ法が「エピソード全体の実現値」という
分散の大きいサンプルから価値を推定するのに対し，動的計画法は既知の遷移確率に基づく
厳密な期待値計算を行うためである．一方で，モンテカルロ法は環境モデルが未知である，
あるいは状態数が膨大で動的計画法が適用できない問題にも適用できるという利点を持つ．
本実験のように，同一の最適方策に(漸近的に)収束することが期待される設定で
両手法を比較することで，「モデルベース手法はデータ効率に優れるが環境モデルの既知性を要求し，
モデルフリー手法は汎用性に優れるがデータ効率で劣る」という強化学習における
基本的なトレードオフを具体的に確認できる．
...
**比較する対象**: 各 $ \epsilon $ における学習曲線の形状と，50,000エピソード終了時点の最終評価．
