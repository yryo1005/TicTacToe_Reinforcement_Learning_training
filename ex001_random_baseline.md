...
- ランダム方策もQ値テーブル(`Q.npy`)として保存し，ex006で他の手法と同じ形式で読み込んで対戦相手に選べるようにすること．
...
価値関数やQ関数の更新は行わない．
このランダム方策を，全ての合法手に同じ価値(0.0)を割り当てたQ値テーブルとしても表現し，
`Q.npy`として保存する(`TabularQAgent`は同点のQ値をランダムに選ぶため，これを読み込めば
一様ランダムな行動選択を再現できる)．これにより，ex006では学習済みの手法とランダム方策を
全く同じ形式(`Q.npy`)で読み込んで対戦相手に選べるようになる．
