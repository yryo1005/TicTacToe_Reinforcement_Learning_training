...
「スイープ」の定義はex002の「2.2 目的関数・更新式」を参照．
(1回のスイープ = 全ての状態を1回ずつ更新する処理)．
...
ex002の方策反復法は，「方策評価(Vを求める)」と「方策改善(方策を更新する)」という
2つの工程を明確に分けて交互に行っていた．
価値反復法はこの2つを1つの式にまとめ，ベルマン最適方程式

$$
V_{k+1}(s) = \max_{a} \sum_{s'} p(s' \mid s, a) \Big[ r(s,a,s') + \gamma \, V_k(s') \Big]
$$

を直接反復する．右辺は，ex002のベルマン期待方程式の $`\pi(s)`$ (方策が指定する1つの行動)を
$`\max_a`$ (その時点で最も良さそうな行動を選ぶ)に置き換えたものである．
言い換えると，「今から一番良い手を打ち続けたらどれだけの価値が得られるか」を，
方策を明示的に持たずに直接計算している．

この更新を，ex002と同じ判定 $`\max_s |V_{k+1}(s) - V_k(s)| < \theta`$ (1回のスイープでの
最大変化量が閾値 $`\theta`$ を下回る)となるまで繰り返す．
方策反復法(ex002)が「固定した方策のもとでの評価が収束するまで待ってから改善する」のに対し，
価値反復法は「1スイープごとに $`\max_a`$ を取ることで評価と改善を同時に行う」という違いがある．

収束した価値関数 $`V(s)`$ から，ex002と同じ考え方(1手先読みして最も価値の高い行動を選ぶ)で

$$
\pi^{*}(s) = \arg\max_{a} \sum_{s'} p(s'\mid s,a)\big[r(s,a,s') + \gamma V(s')\big]
$$

によって最終的な方策(最適方策) $`\pi^{*}`$ を抽出する．
コード上では，`value_iteration`関数内の`new_V[s]`が上式の $`V_{k+1}(s)`$ に，
`best_q`の計算が $`\max_a(\cdot)`$ に対応する．
...
| 項目 | 値 |
| :--- | :--- |
| 環境 | 3目並べ(ex001, ex002と共通) |
| 割引率 $`\gamma`$ | 0.95(ex002と同一) |
| 収束判定閾値 $`\theta`$ | 1e-4(ex002と同一) |
| シード | 0, 1, 2 |
| 結果の保存先 | `outputs/ex003_tictactoe_dp/ValueIteration/gamma0.95_theta0.0001/seed_{k}/` |

価値反復法は $`V`$ の初期値を常に0とするため，理論上シードに依存しない．
それでも実験ループの構造をex002と揃え，結果がシードによらず一致することを確認するために，
同じ3シードを実行する．
...
価値反復法と方策反復法は，いずれもベルマン方程式に基づく動的計画法である．
有限MDPでは，同一の最適価値関数・最適方策に収束することが理論的に保証されている．
本実験でQ値の最大差がほぼ0であったことは，この性質と整合する結果である．

一方で，両手法の反復構造には明確な違いがある．
方策反復法は，固定した方策のもとでの評価が収束するまで内側ループを回す．
そのため外側の改善回数自体は少ないが，1回の改善につき複数回のスイープを要する．
価値反復法は，評価の収束を待たずに $`\max_a`$ を取ることで改善を先取りする．
外側・内側の区別がなく，1スイープの計算はやや重いが，全体では少ないスイープ数で収束する傾向がある．

いずれの手法も，計算量は状態数と行動数の積に比例する．
状態空間が本実験(2,423状態)よりはるかに大きい問題では，全状態を陽に反復すること自体が
非現実的になる．この限界が，サンプルされた経験のみから学習するモデルフリー手法
(ex004・ex005)を導入する動機である．
